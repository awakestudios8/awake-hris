...
let th=0,tt=0,ta=0,tiz=0,tsk=0,tcu=0;em.forEach(e=>{const at=gA(e.id,today,new Date());if(at.st==="Hadir"||at.st==="Terlambat")th++;else if(at.st==="Alpha")ta++;else if(at.st==="Izin")tiz++;else if(at.st==="Sakit")tsk++;else if(at.st==="Cuti")tcu++;if(at.st==="Terlambat")tt++;});
...
return <tr key={r.ds+"-"+r.e.id}><td style={{fontWeight:600}}>{r.d2} {MN[r.mo]}</td><td><div className="er"><div className="av sm" style={{background:AV[r.i%9]}}>{r.e.n[0]}</div>{r.e.n}</div></td><td className="mo">{a.ci||"-"}</td><td className="mo">{a.co||"-"}</td><td><span style={bg(a.st.toLowerCase())}>{a.st}</span></td><td className="mo">{a.oi||"-"}</td><td className="mo">{a.oo||"-"}</td><td>{a.oh>0?<span style={bg(a.wt?"lembur hari libur":"lembur")}>{fj(a.oh)}</span>:"-"}</td></tr>;})}</tbody></table></div>}</div>;};
