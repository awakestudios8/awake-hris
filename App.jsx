...
const sf=async(t,q="")=>{try{const r=await fetch(SUPA+"/rest/v1/"+t+q,{headers:SH});if(!r.ok)return [];return await r.json();}catch(e){return [];}};
const si=async(t,d)=>{try{const r=await fetch(SUPA+"/rest/v1/"+t,{method:"POST",headers:SH,body:JSON.stringify(d)});return await r.json();}catch(e){return null;}};
...
if(checkDt&&checkDt<=today&&r.st==="Alpha"){const w=checkDt.getDay();if(w===0||w===6)return{...r,st:"-"};}
...
const d=dt.getDate();const w=dt.getDay();if(w===0||w===6){const at=gA(ei,d,new Date(dt));if(at.wt)ow=rj(ow+at.oh);continue;}
...
si("attendance",{employee_id:ei,date:dateStr,clock_in:status}).catch(()=>{});};
...
const EDash=()=>{if(!le)return null;const rc=pR(le.id),cu=CQ-cU(le.id),spp=aS(le.id),ml=lv.filter(l=>l.ei===le.id);const prd=pLbl(le.pd);
...
const ACal=()=>{const emp=sls?em.find(e=>e.id===sls):null;const rc=emp?pR(emp.id):null;const prd=emp?pLbl(emp.pd):"";
...
<div className="tw"><table><thead><tr><th>Tgl</th><th>Hari</th><th>Masuk</th><th>Keluar</th><th>Status</th><th>Lembur</th><th>Edit</th></tr></thead><tbody>{(()=>{const pr2=getPR(emp.pd);const ds=[];for(let dt=new Date(pr2.sd);dt<=pr2.ed;dt.setDate(dt.getDate()+1)){ds.push(new Date(dt));}return ds.map(dt=>{const d=dt.getDate();const mo=dt.getMonth();const w=dt.getDay();const dn=["Min","Sen","Sel","Rab","Kam","Jum","Sab"][w];const a=gA(emp.id,d);const we=w===0||w===6;if(we&&!a.wt)return <tr key={dt.toISOString()} style={{opacity:.25}}><td>{d} {MN[mo]}</td><td style={{color:BR}}>{dn}</td><td colSpan={5} style={{color:"#cbd5e1"}}>Libur</td></tr>;const ds2=dt.toISOString().slice(0,10);const isFut=new Date(dt)>new Date(new Date().setHours(0,0,0,0));
...
const EAtt=()=>{if(!le)return null;return <div className="cd"><div className="ch"><span className="ct">Kehadiran — April 2026</span></div><div style={{fontSize:12,color:"#94a3b8",marginBottom:10}}>Periode: <strong style={{color:"#0f172a"}}>{pLbl(le.pd)}</strong></div><div className="tw"><table><thead><tr><th>Tanggal</th><th>Hari</th><th>Masuk</th><th>Keluar</th><th>Status</th><th>Lembur</th></tr></thead><tbody>{Array.from({length:30},(_,i)=>i+1).reverse().map(d=>{const w=new Date(2026,3,d).getDay(),dn=["Min","Sen","Sel","Rab","Kam","Jum","Sab"][w],a=gA(le.id,d),we=w===0||w===6;if(we&&!a.wt)return null;return <tr key={d}><td style={{fontWeight:600}}>{d} April</td><td style={{color:we?BR:"inherit"}}>{dn}</td><td className="mo">{a.ci||"-"}</td><td className="mo">{a.co||"-"}</td><td><span style={bg(a.st)}>{a.st}</span></td><td>{a.oh>0?<span style={bg(a.wt?"lembur hari libur":"lembur")}>{fj(a.oh)}{a.obk>0?" (-"+a.obk+"m istirahat)":""}</span>:"-"}</td></tr>;}).filter(Boolean)}</tbody></table></div></div>;};
